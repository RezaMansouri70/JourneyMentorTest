...
namespace IntegerationTest.BookServices
{
    public class AriportServicesTest : IClassFixture<ApplicationTestFixture>
    {
        ...
        [Fact]
        public void should_get_airports_from_the_source()
        {
            // Act
            var result = _manager.GetAirports(new Application.Models.General.Filter() { PageIndex = 1, PageSize = 1 }).GetAwaiter().GetResult();
